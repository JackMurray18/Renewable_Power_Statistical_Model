          ...
          for(A5 in seq(from=.1,to=.4,by=.1))
          {
            ...
            for(i in seq(from=1,to=nrow(location1),by=1))
            {
             pred.sin.days[i] <- ((A2*cos((pi*location1$total.days[i])/91.25))+
                                  ...
                                  (A5*sin((pi*location1$total.days[i])/22.8125))+
                                  (A6*cos((pi*location1$total.days[i])/22.8125))+
                                  C)
...
### Notes
  + Taking the Log of wind speed increases RMSE and make the data less normal
  + a period of just 1 year works really well
    + multi-year periods seem to have no effect (small coef) or introduce a lot of error (larger coef)
    + less than half year periods also have limited benefit and are fitted to have small
  + sine terms have the largest effect - 
    + Tend to be fitted with the largest values than cosine terms